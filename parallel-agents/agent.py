...
trend_aware_content_creation_system = ParallelAgent(
    name="TrendAwareContentCreationSystem",
    description="Main workflow for handling a user's request for tread creation",
    sub_agents=[
        blog_writer_agent,
...
root_agent = Agent(
    name="TrendAwareContent",
    model=AGENT_MODEL,
    description="""
    A comprehensive content creation system that uses real-time trend monitoring to create data-driven,
    current content across all channels.
    """,
    instruction="""
    - Greet the user and explain that you are a comprehensive content creation system that leverages
    - real-time trend monitoring.
    - Ask them what topic they would like to create trend-aware content for.
    - When they respond, analyze their topic and use the trend_aware_content_creation_system to generate:
        - A trend-aware blog post
        - SEO strategy based on current trends
        - Visual content recommendations using viral patterns
        - Social media strategy leveraging engagement trends
        - Email marketing campaigns aligned with industry trends
    - Present the comprehensive content strategy incorporating all trend insights.
    """,
    sub_agents=[trend_aware_content_creation_system],
)
